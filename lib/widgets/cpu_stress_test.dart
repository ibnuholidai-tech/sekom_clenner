...
class _CpuStressTestState extends State<CpuStressTest> {
  ...
  static void _cpuStressWork(Map<String, dynamic> params) {
    ...
    bool shouldRun = true;
    bool isReduced = params['reduced'] ?? false;
    ...
    while (shouldRun) {
      var result = 0;
      int iterations = isReduced ? 500000 : 1000000;
      int checkInterval = isReduced ? 50000 : 100000;
  ...
  static void _memoryStressWork(Map<String, dynamic> params) {
    ...
    if (isReduced) {
      blockSize = (blockSize * 0.6).round();
      maxBlocks = (maxBlocks * 0.7).round();
    }
    ...
    while (shouldRun) {
      try {
        if (memoryBlocks.length < maxBlocks) {
          ...
          int writeInterval = isReduced ? 8192 : 4096;
          
          switch (pattern) {
            case 'random':
              for (int i = 0; i < block.length; i += writeInterval) {
                block[i] = (DateTime.now().millisecondsSinceEpoch % 256);
              }
              break;
            case 'sequential':
              for (int i = 0; i < block.length; i += (writeInterval * 2)) {
                block[i] = (i % 256);
              }
              break;
            case 'intensive':
              for (int i = 0; i < block.length; i += (isReduced ? 2048 : 512)) {
                block[i] = ((i * 31) % 256);
                if (i % 4096 == 0 && !shouldRun) break;
              }
              break;
            case 'fragmented':
              for (int i = 0; i < block.length; i += (writeInterval * 2)) {
          ...
          if (pattern == 'intensive' && !isReduced) {
            for (int i = 0; i < memoryBlocks.length; i++) {
              var currentBlock = memoryBlocks[i];
              for (int j = 0; j < currentBlock.length; j += 32768) {
        ...
        int delayMs = isReduced ? 100 : (pattern == 'intensive' ? 10 : 50);
  ...
  Future<void> _startCpuTest() async {
    ...
    try {
      ...
      bool isReduced = _isMemoryTesting;
      int coreLimit = isReduced ? (_coreCount * 0.7).round() : _coreCount;
      if (coreLimit < 2) coreLimit = 2;
      ...
      for (int i = 0; i < coreLimit; i++) {
        ...
        await Future.delayed(Duration(milliseconds: isReduced ? 200 : 100));
      }

      setState(() {
        _cpuStatus = 'CPU Stressing $coreLimit cores${isReduced ? ' (Balanced Mode)' : ''}... (${_cpuElapsedSeconds}s)';
      });
  ...
  void _stopCpuTest() {
    ...
    try {
      ...
      if (_cpuElapsedSeconds > 0) {
        TestSummary.saveCpu(seconds: _cpuElapsedSeconds);
      }
    } catch (e) {
  ...
  Future<void> _startMemoryTest() async {
    ...
    try {
      ...
      if (isReduced) {
        isolateCount = (isolateCount * 0.6).round();
        if (isolateCount < 2) isolateCount = 2;
      }
      ...
      for (int i = 0; i < isolateCount; i++) {
        ...
        await Future.delayed(Duration(milliseconds: isReduced ? 400 : 200));
      }

      setState(() {
        _memoryStatus = 'Memory Stressing ($_memoryIntensity)${isReduced ? ' (Balanced Mode)' : ''}... (${_memoryElapsedSeconds}s)';
      });
      ...
      _memoryTimer = Timer.periodic(const Duration(seconds: 1), (timer) async {
        ...
        setState(() {
          ...
          _memoryStatus = 'Memory Stressing ($_memoryIntensity)${isReduced ? ' (Balanced)' : ''}... ${_memoryUsagePercent.toStringAsFixed(1)}% (${_memoryElapsedSeconds}s)';
        });
      });
                ...
                Container(
                  ...
                  child: const Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          'Balanced Mode Active: Tests running simultaneously with reduced intensity',
