...
; -------------------------------
; Optional: Code signing settings
; -------------------------------
; To enable code signing of the installer (and uninstaller), provide a valid SignTool
; and uncomment the lines below. This improves SmartScreen/AV reputation.
; Example with Microsoft SignTool (from Windows SDK):
; SignTool=62
; SignTool=cmd /c "signtool sign /fd SHA256 /tr http://timestamp.digicert.com /td SHA256 /a /n \"Sekom\" $f"
; SignedUninstaller=yes
; SignToolRetryCount=3
; SignToolRetryDelay=2000
; SignToolMinimumFileSize=8192
; NOTE:
; - Replace /n "Sekom" with your certificate subject name, or use /f "path\to\cert.pfx" /p "password"
; - You can also add post-build signing for app binaries (sekom_clenner.exe, SekomHelper.exe) in your CI/CD.

; Outputs
OutputDir=dist\installer
OutputBaseFilename=sekom_cleanner-{#MyAppVersion}
...
; Sealed Flutter Windows release output (copy all, but exclude plugin DLLs to relocate, and map/debug files)
Source: "..\..\build\windows\x64\runner\Release\*"; DestDir: "{app}"; Excludes: "*.pdb;*.map;*_plugin.dll;WebView2Loader.dll"; Flags: recursesubdirs createallsubdirs ignoreversion

; Move plugin DLLs to {app}\data (root stays clean: only exe + flutter_windows.dll + data\)
Source: "..\..\build\windows\x64\runner\Release\*_plugin.dll"; DestDir: "{app}\data"; Flags: ignoreversion
Source: "..\..\build\windows\x64\runner\Release\WebView2Loader.dll"; DestDir: "{app}\data"; Flags: ignoreversion
